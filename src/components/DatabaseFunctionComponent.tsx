import { fetchDatabaseFunction } from "@/services/databaseService"
...
type DatabaseFunctionComponentProps = {
    currentConnection: DatabaseConnection | null
}

export default function DatabaseFunctionComponent(props: DatabaseFunctionComponentProps) {
    ...
    const loadTableData = async (dbName: string): Promise<string> => {
        if (props.currentConnection) {
            const data = await fetchDatabaseFunction({ connection: props.currentConnection, databaseName: dbName })
            setRows(data)
            const listColumn: FieldDetail[] = []
            if (data.length > 0) {
                const listName = Object.keys(data[0])
                listName.forEach(name => {
                    listColumn.push({ fieldName: name, fieldType: "" })
                })
            }
            else listColumn.push()
    ...
    return (
        <div className="grid grid-cols-12 border-t" style={{ height: '100%' }}>
            {!props.currentConnection ? <></> : !loading ?
                ...
                <>
                    <DisplayDatabaseBar currentConnection={props.currentConnection} handleDatabaseAction={loadTableData} isNotExpaned />
                    <div className="col-span-10" style={{ overflow: 'auto' }}>
                        {rows.length !== 0 ?
                            ...
                            : <div className="flex justify-center">Database này chưa có Function nào</div>
                        }
                    </div>
                </>
            }
