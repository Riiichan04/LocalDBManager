import { fetchDatabaseViews } from "@/services/databaseService"
...
export default function DatabaseViewComponent(props: DatabaseViewComponentProps) {
    ...
    const loadTableData = async (dbName: string): Promise<string> => {
        if (props.currentConnection) {
            const data = await fetchDatabaseViews({ connection: props.currentConnection, databaseName: dbName })
            setRows(data)
